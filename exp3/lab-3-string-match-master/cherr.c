...
TimeCount wm_pattern_matching(PatternSet *set) {
    nfound = 0;
    nline = 1;
    FILE *ft;
    char text[4096];
    ...
    tc.Itime = endtv.tv_sec * 1000 + endtv.tv_usec / 1000 - starttv.tv_sec * 1000 - starttv.tv_usec / 1000;
    ...
    tc.Stime = endtv.tv_sec * 1000 + endtv.tv_usec / 1000 - starttv.tv_sec * 1000 - starttv.tv_usec / 1000;
...
TimeCount ac_pattern_matching(PatternSet *set) {
    nfound = 0;
    nline = 1;
    FILE *ft;
    char text[4096];
...
int main(int argc, char *argv[]) {
    ...
    char *PatternFileName = "pattern1w.txt";
    ...
    for(int p=0;p<30;p++){
        ...
        if (method_choice == 1) {
            ...
            PatternSet short_set = {short_patterns, short_count, TextFileName};
            PatternSet long_set = {long_patterns, long_count, TextFileName};
            wm_tc=wm_pattern_matching(&long_set);
            ac_tc=ac_pattern_matching(&short_set);
        }

        // 输出时间
        ac_itime[p]=ac_tc.Itime;
        ac_stime[p]=ac_tc.Stime;
        wm_itime[p]=wm_tc.Itime;
        wm_stime[p]=wm_tc.Stime;


        // 释放模式串内存
        if (method_choice == 1 || method_choice == 3) {
            for (int i = 0; i < short_count; i++) {
                if(short_patterns[i]!=NULL){
                    free(short_patterns[i]);
                }
            }
            if(short_patterns!=NULL){
                free(short_patterns);
            }
        }
        if (method_choice == 2 || method_choice == 3) {
            for (int i = 0; i < long_count; i++) {
                if(long_patterns[i]!=NULL){
                    free(long_patterns[i]);
                }
            }
            if(long_patterns!=NULL){
                free(long_patterns);
            }
        }
    }
    printf("=================");
    for(int i=0;i<30;i++){
        printf("epoch: %2d time: %3ld %3ld %3ld %3ld \n",i, ac_itime[i],ac_stime[i],wm_itime[i],wm_stime[i]);
    }
    printf("=================");

    for(int i=0;i<30;i++){
        printf("epoch: %2d total_time: %5ld %5ld \n",i, ac_itime[i]+ac_stime[i],wm_itime[i]+wm_stime[i]);
    }
